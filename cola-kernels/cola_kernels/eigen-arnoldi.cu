...
__global__ void update_new_vector(cfloat A, cuFloatComplex *B, cuFloatComplex *C, int N, int M, int i);
...
int main(int argc, char *argv[])
{
    ...
    norm = sqrt(norm);
    std::cout << "(" << norm.real() << ", " << norm.imag() << "i)" << std::endl;
    ...
    // for (int i = 0; i < N; i++)
    // {
    //     std::cout << "(" << Q[index(i, 0, (max_iters + 1))].real() << ", " << Q[index(i, 0, (max_iters + 1))].imag() << "i)" << std::endl;
    // }
    while (idx < max_iters)
    {
        ...
        //     std::cout << "(" << new_vector[i].real() << ", " << new_vector[i].imag() << "i)" << std::endl;
        // }
        cudaMemcpy(h_vec_d, h_vec, (max_iters + 1) * sizeof(cuFloatComplex), cudaMemcpyHostToDevice);
        for (int j = 0; j < idx + 1; j++)
        {
            cudaMemcpy(ang_d, ang, N * sizeof(cuFloatComplex), cudaMemcpyHostToDevice);
            angle_cal<<<(N + 255) / 256, 256>>>(Q_d, new_vector_d, ang_d, N, (max_iters + 1), j);
            cudaMemcpy(ang, ang_d, N * sizeof(cuFloatComplex), cudaMemcpyDeviceToHost);
            //     for (int i = 0; i < N; i++)
            // {
            //     std::cout << "(" << ang[i].real() << ", " << ang[i].imag() << "i)" << std::endl;
            // }
            ...
            update_new_vector<<<(N + 255) / 256, 256>>>(angle, Q_d, new_vector_d, N, (max_iters + 1), j);
        }
        cudaMemset(new_vector_d_r, 0, (N + 255) / 256 * sizeof(cuFloatComplex));
        memset(new_vector_r, 0, (N + 255) / 256 * sizeof(cfloat));
        norm_cal<<<(N + 255) / 256, 256>>>(new_vector_d, new_vector_d_r, N);
        ...
        norm = sqrt(norm);
        std::cout << "(" << norm.real() << ", " << norm.imag() << "i)" << std::endl;
        if (std::abs(norm.real()) < 0.0000001)
        {
            norm.real(0.0000001);
            norm.imag(0.0f);
        }
        ...
        update_arr<<<(N + 255) / 256, 256>>>(H_d, h_vec_d, N, max_iters, idx);
...
__global__ void update_new_vector(cfloat A, cuFloatComplex *B, cuFloatComplex *C, int N, int M, int i)
{
    ...
    if (row_idx < N)
    {
        cuFloatComplex a = make_cuFloatComplex(A.real(), A.imag());
        int idx = index(row_idx, i, M);
        C[row_idx] = cuCsubf(C[row_idx], cuCmulf(a, B[idx]));
    }
}
